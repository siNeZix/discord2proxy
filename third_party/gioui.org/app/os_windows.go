...
type window struct {
	...
	loop      *eventLoop
	// placed reports whether the window has been positioned at least once.
	// It is used to center the window on its first windowed Configure so it
	// never appears at the OS default top-left corner and visibly jumps.
	placed bool
}
...
func (w *window) Configure(options []Option) {
	...
	case Windowed:
		...
		// On the first windowed placement, center the window on its monitor
		// before it is shown. Without this the window is created at
		// CW_USEDEFAULT (top-left) and visibly jumps when the application
		// re-centers it after the first frame.
		if !w.placed {
			w.placed = true
